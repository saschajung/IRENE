...
prior <- read.table(paste0("../../NK_Newest/iPSC_GSM1088317_TPM_bool_ecdf.csv"),header=TRUE,stringsAsFactors = FALSE)
...
setwd("~/Downloads/core_networks/output_files/")
...
while(TRUE){
  ...
  for(idx in pert_idx){
    zeros <- which(line_vec[1:(length(line_vec)-1)] == 0)
    ones <- setdiff(which(line_vec[1:(length(line_vec)-1)] == 1),which(perturbagens$V1 %in% pp[,idx]))
    stateProb <- prod(1-prior$ecdf[zeros])*prod(prior$ecdf[ones])
    chromChangeProb <- 1#prod((commonEnhancers[zeros]/totalEnhancers[zeros]))*prod(1-(commonEnhancers[ones]/totalEnhancers[ones]))
    prob <- tail(line_vec,1)#ifelse(is.na(tail(line_vec,1)),0,1/tail(line_vec,1))
...
wTP53 <- c(wTP53,nd[apply(pp,2,function(x){any(levels(pp)[x] == "TP53")})])
woTP53 <- c(woTP53,nd[!apply(pp,2,function(x){any(levels(pp)[x] == "TP53")})])

wTP53 <- c()
woTP53 <- c()
save(nominator,denominator,pp,file = paste0("./FinalVals_6_",args[1],".RData"))
